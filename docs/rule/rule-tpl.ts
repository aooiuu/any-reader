export interface Rule {
  ...
  host: string; // [!code highlight]
  id: string; // uuid, 用于区分规则的唯一性
  name: string; // 规则名称
  sort: number; // 规则排序, 越高越靠前
  contentType: ContentType; // 规则类型 // [!code highlight]
  loadJs: string; // 全局JS脚本
  author: string; // 规则作者
  userAgent: string; // Headers JSON字符串

  // ===== 解析流程 - 搜索 =====
  enableSearch: boolean; // 表示搜索功能开启的状态 // [!code highlight]
  searchUrl: string; // 用法可参考 `URL地址规则` // [!code highlight]
  // 以下规则用法可参考 `取列表规则`
  // 可以使用变量: `result` 获取 `searchUrl` 的结果
  searchList: string; // [!code highlight]
  // 以下规则用法可参考 `取内容规则`
  // 可以使用变量: `result` 获取 `searchList` 数组当前项的结果
  searchCover: string; // 封面
  searchName: string; // 标题 // [!code highlight]
  searchAuthor: string; // 作者
  searchChapter: string; // 最新章节
  searchDescription: string; // 描述
  searchResult: string; // 结果 // [!code highlight]
  ...
  chapterList: string; // 列表 // [!code highlight]
  // 以下规则用法可参考 `取内容规则`
  // 可以使用变量: `result` 获取 `chapterList` 数组当前项的结果
  // 可以使用变量: `lastResult` 获取 `searchResult` 或者 `discoverResult` 的结果
  chapterName: string; // 标题 // [!code highlight]
  chapterCover: string; // 封面
  chapterTime: string; // 时间
  chapterNextUrl: string; // 下一页地址
  chapterResult: string; // 结果 // [!code highlight]

  // ===== 解析流程 - 发现页 =====
  enableDiscover: boolean; // 是否启用发现页 // [!code highlight]
  discoverUrl: string; // 用法可参考 `发现页分类规则` // [!code highlight]
  // 以下规则用法可参考 `取列表规则`
  // 可以使用变量: `result` 获取 `discoverUrl` 的结果
  discoverList: string; // 列表 // [!code highlight]
  // 以下规则用法可参考 `取内容规则`
  // 可以使用变量: `result` 获取 `discoverList` 数组当前项的结果
  discoverName: string; // 标题 // [!code highlight]
  discoverCover: string; // 封面
  discoverAuthor: string; // 作者
  discoverDescription: string; // 描述
  discoverResult: string; // 结果 // [!code highlight]
  ...
  // ===== 解析流程 - 正文 =====
  contentUrl: string;
  contentItems: string; // 内容 // [!code highlight]
